    ...
    for _ in range(target_merges):
        ...
        for pair, count in pair_counts.items():
            ...
            if (count > best_count or 
                (count == best_count and tie_breaker > best_tie_breaker)):
                best_pair = pair
                best_count = count
...
def encode(text: str, vocab: dict[int, bytes], merges: list[tuple[bytes, bytes]], special_tokens: list[str] | None = None) -> list[int]:
    """
    Encodes text using the trained BPE tokenizer.
    
    Args:
        text: Input text to encode
        vocab: Vocabulary from train_bpe
        merges: Merge rules from train_bpe
        ...
        for part in parts:
            if part in special_tokens:
                ...
                if special_token_bytes in bytes_to_id:
                    result.append(bytes_to_id[special_token_bytes])
            elif part:  # Non-empty regular text
                # Encode using BPE
                result.extend(_encode_text_part(part, bytes_to_id, merges))
    else:
        result.extend(_encode_text_part(text, bytes_to_id, merges))
    
    return result


def _encode_text_part(text: str, bytes_to_id: dict[bytes, int], merges: list[tuple[bytes, bytes]]) -> list[int]:
    """
    Encode a text part using BPE (without special token handling).
    """
    # Pre-tokenize the text first
    PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""
    pat = re.compile(PAT)
    
    result = []
    
    # Process each pre-token separately
    for match in pat.finditer(text):
        ...
        for merge_rule in merges:
            token1, token2 = merge_rule
            merged_token = token1 + token2
            
            if merged_token not in bytes_to_id:
                continue
        ...
        for token in tokens:
            if token in bytes_to_id:
                result.append(bytes_to_id[token])
...
class BPETokenizer:
    ...
    def encode(self, text: str) -> list[int]:
        ...
        Returns:
            List of token IDs
        """
        return encode(text, self.vocab, self.merges, self.special_tokens)
