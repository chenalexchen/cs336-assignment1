...
fn parse_args() -> Result<Args, Box<dyn std::error::Error>> {
    ...
    if args.len() < 4 {
        ...
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  {} data/owt_train.txt 32000 tokenizer_output/owt_32k", args[0]);
        eprintln!("  {} data/owt_train.txt 50000 tokenizer_output/owt_50k \"<|endoftext|>\" \"<|pad|>\"", args[0]);
        std::process::exit(1);
    }
...
fn save_tokenizer_outputs(
    ...
    writeln!(stats_file, "BPE Training Statistics")?;
    writeln!(stats_file, "=======================")?;
    writeln!(stats_file)?;
    ...
    for (&id, bytes) in vocab {
        if id < 256 {
            byte_tokens += 1;
        } else if args.special_tokens.contains(&String::from_utf8_lossy(bytes).to_string()) {
            special_tokens += 1;
        } else {
            merged_tokens += 1;
        }
    ...
    writeln!(stats_file, "Vocabulary Composition:")?;
    writeln!(stats_file, "- Base byte tokens (0-255): {}", byte_tokens)?;
    writeln!(stats_file, "- Special tokens: {}", special_tokens)?;
    writeln!(stats_file, "- Learned merge tokens: {}", merged_tokens)?;
    writeln!(stats_file, "- Total tokens: {}", vocab.len())?;
    ...
    if let Ok(metadata) = std::fs::metadata(&args.input_file) {
        ...
        writeln!(stats_file)?;
        writeln!(stats_file, "Input File Info:")?;
        if file_size_gb >= 1.0 {
            writeln!(stats_file, "- Size: {:.1} GB", file_size_gb)?;
        } else {
            writeln!(stats_file, "- Size: {:.1} MB", file_size_mb)?;
        }
    }
